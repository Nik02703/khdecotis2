import { NextResponse } from 'next/server';
import connectToDatabase from '@/lib/mongoose';
import Order from '@/models/Order';
import { decodeCallbackPayload } from '@/lib/phonepe';

/**
 * POST /api/payment/phonepe-callback
 *
 * PhonePe V2 Server-To-Server Callback (Webhook).
 *
 * V2 sends either:
 *   - A Base64 encoded JSON payload in body.response
 *   - Or a direct JSON body with event + payload fields
 *
 * ALWAYS return HTTP 200 — PhonePe retries if it gets anything else.
 */
export async function POST(req) {
  try {
    const body = await req.json();
    console.log('[PhonePe Callback] Raw body keys:', Object.keys(body));

    // ── Try to extract transaction details ──
    let merchantTransactionId = null;
    let state = null;
    let paymentCode = null;
    let phonePeTxnId = '';
    let decoded = body;

    // Format 1: V1-style { response: "<base64>" }
    if (body.response && typeof body.response === 'string') {
      decoded = decodeCallbackPayload(body.response);
      if (!decoded) {
        console.error('[PhonePe Callback] Failed to decode base64 response');
        return NextResponse.json({ success: true }, { status: 200 });
      }
    }

    // Format 2: V2-style { event: "checkout.order.completed", payload: { ... } }
    if (decoded?.event && decoded?.payload) {
      const p = decoded.payload;
      merchantTransactionId = p.merchantOrderId || p.merchantTransactionId || p.transactionId;
      state = p.state;
      paymentCode = decoded.event; // e.g. "checkout.order.completed"
      phonePeTxnId = p.providerReferenceId || p.transactionId || '';

      // Map V2 event names to states
      if (decoded.event === 'checkout.order.completed') state = 'COMPLETED';
      if (decoded.event === 'checkout.order.failed') state = 'FAILED';
    }
    // Format 3: V1-style decoded { code, data: { merchantTransactionId, state } }
    else if (decoded?.data?.merchantTransactionId || decoded?.code) {
      merchantTransactionId = decoded?.data?.merchantTransactionId;
      state = decoded?.data?.state;
      paymentCode = decoded?.code;
      phonePeTxnId = decoded?.data?.transactionId || '';
    }
    // Format 4: Direct flat fields
    else if (decoded?.merchantOrderId || decoded?.merchantTransactionId) {
      merchantTransactionId = decoded.merchantOrderId || decoded.merchantTransactionId;
      state = decoded.state;
      phonePeTxnId = decoded.transactionId || decoded.providerReferenceId || '';
    }

    console.log('[PhonePe Callback] Transaction:', merchantTransactionId, '| State:', state, '| Event:', paymentCode);

    if (!merchantTransactionId) {
      console.warn('[PhonePe Callback] No merchantTransactionId/merchantOrderId in payload');
      return NextResponse.json({ success: true }, { status: 200 });
    }

    const db = await connectToDatabase();
    if (!db) {
      console.error('[PhonePe Callback] DB connection failed');
      return NextResponse.json({ success: true }, { status: 200 });
    }

    // Search by merchantTransactionId (which is our merchantOrderId)
    const orderDoc = await Order.findOne({ merchantTransactionId });

    if (orderDoc) {
      if (state === 'COMPLETED' || paymentCode === 'PAYMENT_SUCCESS' || paymentCode === 'checkout.order.completed') {
        if (orderDoc.paymentStatus !== 'paid') {
          orderDoc.paymentStatus = 'paid';
          orderDoc.paymentMethod = 'PhonePe';
          orderDoc.paymentTransactionId = phonePeTxnId;
          orderDoc.paidAt = new Date();
          orderDoc.status = 'Pending';
          orderDoc.color = '#dcfce7';
          orderDoc.text = '#16a34a';
          orderDoc.phonePeResponse = decoded;
          await orderDoc.save();
          console.log('[PhonePe Callback] Order', orderDoc.orderId, 'marked as PAID');
        }
      } else if (state === 'FAILED' || paymentCode === 'PAYMENT_ERROR' || paymentCode === 'checkout.order.failed') {
        if (orderDoc.paymentStatus !== 'failed') {
          orderDoc.paymentStatus = 'failed';
          orderDoc.phonePeResponse = decoded;
          await orderDoc.save();
          console.log('[PhonePe Callback] Order', orderDoc.orderId, 'marked as FAILED');
        }
      } else {
        console.log('[PhonePe Callback] Order', orderDoc.orderId, 'state:', state, '— no update');
      }
    } else {
      console.warn('[PhonePe Callback] Order not found for merchantTransactionId:', merchantTransactionId);
    }

    // Always return 200
    return NextResponse.json({ success: true }, { status: 200 });

  } catch (error) {
    console.error('[PhonePe Callback] ❌ Exception:', error?.message);
    return NextResponse.json({ success: true }, { status: 200 });
  }
}
